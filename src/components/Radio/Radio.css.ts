...
export const dotBase = style({
  ...
  selectors: { [`${input}:checked + ${boxBase} &`]: { opacity: 1 } },
});
...
export const group = recipe({
  base: { display: "flex", gap: vars.space.sm, border: "none", margin: 0, padding: 0 },
  variants: { orientation: { vertical: { flexDirection: "column" }, horizontal: { flexDirection: "row", flexWrap: "wrap" } } },
  defaultVariants: { orientation: "vertical" },
});

export type GroupVariants = NonNullable<RecipeVariants<typeof group>>;
